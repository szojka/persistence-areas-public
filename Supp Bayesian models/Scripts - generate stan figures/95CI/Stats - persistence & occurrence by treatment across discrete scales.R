...
#------------------------------------------------------------------------------
# DESCRIPTION: Figure 2 Bayes version
#------------------------------------------------------------------------------
...
########################
{
  ...
  fig2A_per_no <- fig2A_per %>% 
    filter(persistence %in% "no") %>% # idea is to keep zeros, so find where persistence no = 1, and make the prop zero, then rbind to persistence yes dataframe
    filter(prop == 1) %>%
    select(-persistence)
  ...
  # temp_key <- nkey %>%
  #   select(block, site, grid) %>%
  #   distinct()
  # fig2A_per <- left_join(fig2A_per, temp_key, by = 'block') # join to account for random effects
  # 
...
load(here::here("Supp Bayesian models/Stan rdata/brms 95CI/m.plot_brm.rdata"))
...
load(here::here("Supp Bayesian models/Stan rdata/brms 95CI/m.grid_brm.rdata"))

# check model fit:
summary(m.grid_brm) # bulk ess > 800 and Rhat = 1.00
...
load(here::here("Supp Bayesian models/Stan rdata/brms 95CI/m.site_brm.rdata"))
...
# add pairwise comparison table, response should mirror predictions above ^
em <- emmeans(m.site_brm, ~treatment, type = "response")  
