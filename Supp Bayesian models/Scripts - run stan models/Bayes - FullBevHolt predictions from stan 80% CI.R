...
# Predictions from full Beverton-Holt model, using 80% credible intervals 
...
dat_ab <- grid_sum_dat %>%
  ...
  mutate(y_hat_lower = quantile(site_sum, probs = 0.1)) %>%
  mutate(y_hat_upper = quantile(site_sum, probs = 0.9)) %>%
  ungroup() %>%
...
# pred_dat_saved
# write csv
save(pred_data_saved, file = here::here("Supp Bayesian models/Stan rdata/fullBH_pred_data_saved_80CI.rdata"))# cluster

# grid_pred
save(grid_pred, file = here::here("Supp Bayesian models/Stan rdata/fullBH_grid_pred_80CI.rdata"))# cluster

# site_pred
save(site_pred, file = here::here("Supp Bayesian models/Stan rdata/fullBH_site_pred_80CI.rdata"))# cluster
