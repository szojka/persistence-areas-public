...
# start with Bromus
focal<-as.data.frame(subset(dat_final_doubled,species=="brohor"))
#head(focal)
...
block_number <- length(unique(focal$block_stan_id)) # 386 entries

# key for which block original ID corresponds to brohors' stan ID:
brohor_block_key <- focal %>%
...
PrelimFit <- stan(file = here::here("Supp Bayesian models/Stan models/BH - nested.stan"), data = c("Fecundity", "N", "Abundance", "Cover", "P", "Site", "G","Grid","Env"),iter = 20000, chains = 4, thin = 2) # , init = initials1

Brohor_posteriors <- PrelimFit # every parameter converged

# when running on cluster:
save(Brohor_posteriors, file = here::here("Supp Bayesian models/Stan rdata/Posteriors/Brohor_posteriors.rdata"))
