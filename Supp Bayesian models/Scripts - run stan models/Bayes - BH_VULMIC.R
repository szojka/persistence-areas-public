...
# Vulmic:
focal<-as.data.frame(subset(dat_final_doubled,species=="vulmic"))
#head(focal)
...
block_number <- length(unique(focal$block_stan_id)) # 413 entries

# key for which block original ID corresponds to vulmics' stan ID:
vulmic_block_key <- focal %>%
...
Vulmic_posteriors <- PrelimFit # every parameter converged

save(Vulmic_posteriors, file = here::here("Supp Bayesian models/Stan rdata/Posteriors/Vulmic_posteriors.rdata"))
