...
# plaere:
focal<-as.data.frame(subset(dat_final_doubled,species=="plaere"))
#head(focal)
...
# key for which block original ID corresponds to plaeres' stan ID:
plaere_block_key <- focal %>%
...
Plaere_posteriors <- PrelimFit # every parameter converged

save(Plaere_posteriors, here::here("Supp Bayesian models/Stan rdata/Posteriors/Plaere_posteriors.rdata"))
