...
# Predictions correct for potential biases, using 95% credible intervals 

# this script does not assess model fits at all, assuming all is well, it simply works the posteriors into dataframes used for my figures and analyses in persistence-areas repo.
...
dat_a <- grid_sum_dat_A %>%
  ...
  mutate(y_hat_baseline_lower = quantile(grid_sum, probs = 0.025)) %>%
  mutate(y_hat_baseline_upper = quantile(grid_sum, probs = 0.975)) %>%
  ungroup() %>%
...
dat_b <- grid_sum_dat_B %>%
  ...
  mutate(y_hat_no_intra_lower = quantile(grid_sum, probs = 0.025)) %>%
  mutate(y_hat_no_intra_upper = quantile(grid_sum, probs = 0.975)) %>%
  ungroup() %>%
...
dat_a <- site_sum_dat_A %>%
  ...
  mutate(y_hat_baseline_lower = quantile(site_sum, probs = 0.025)) %>%
  mutate(y_hat_baseline_upper = quantile(site_sum, probs = 0.975)) %>%
  ungroup() %>%
...
dat_b <- site_sum_dat_B %>%
  ...
  mutate(y_hat_no_intra_lower = quantile(site_sum, probs = 0.025)) %>%
  mutate(y_hat_no_intra_upper = quantile(site_sum, probs = 0.975)) %>%
  ungroup() %>%
...
# pred_dat_saved
# write csv
save(pred_data_saved, file = here::here("Supp Bayesian models/Stan rdata/pred_data_saved_95CI.rdata"))# cluster

# grid_pred
save(grid_pred, file = here::here("Supp Bayesian models/Stan rdata/grid_pred_95CI.rdata"))# cluster

# site_pred
save(site_pred, file = here::here("Supp Bayesian models/Stan rdata/site_pred_95CI.rdata"))# cluster
