...
source(here::here("Persistence thresholds/Supp Bayesian models/Scripts - generate stan figures/95CI/Stats - SAR-PAR categories across discrete scales - i. averaged.R")) # loads main source too

# ii. ACCUMULATED METHOD OF PERSISTENCE
source(here::here("Persistence thresholds/Supp Bayesian models/Scripts - generate stan figures/95CI/Stats - SAR-PAR categories across discrete scales - ii. accumulated.R")) # loads main source too
...
par_dat <- ggplot(vis) + 
  ...
  scale_shape_manual(values = c("Without neighbors" = 2,
                                "With neighbors" = 19)) +
    guides(color = guide_legend(title = "", order = 1, nrow = 3),
...
#---------------------------------------------------------------------------
library(patchwork)
png("Persistence thresholds/Figures/Bayes/95CI/discrete_SAR-PAR_full.png", height = 5, width = 7, units = "in", res=800)
par_dat 
dev.off()
#---------------------------------------------------------------------------
