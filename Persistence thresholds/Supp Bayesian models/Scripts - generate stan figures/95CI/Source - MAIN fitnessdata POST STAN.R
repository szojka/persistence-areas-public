...
tag_labels <- read_csv("Persistence thresholds/Data/Tag labels.csv")

# Should retain order such that unique key (names) is maintained for each group

# make a key to connect 'names' to 'seeds_clean' 
...
load(here::here("Persistence thresholds/Supp Bayesian models/Stan rdata/pred_data_saved_95CI.rdata"))
...
# HERE PERSISTENCE HAPPENS IF CI TOUCHES 1

plotlev$persistence <- NA
plotlev$persistence[plotlev$predicted_fecundity_upr >= 1] <- c("yes")
plotlev$persistence[plotlev$predicted_fecundity_upr < 1] <- c("no")
plotlev$persistence[is.na(plotlev$persist_final)] <- c("no")

# also identify how many instances CI's overlap 1
overlap_dat <- data.frame(overlap = NA, species = NA)
for(i in 1:length(plotlev$tube)){
  overlap_dat[i, 1] <- plotlev$predicted_fecundity_lwr[i] <= 1 & 
    1 <= plotlev$predicted_fecundity_upr[i]
}
changed <- filter(overlap_dat, overlap == TRUE)[1]
length(changed$overlap)/length(plotlev$tube)*100 # 818/5718, 14.3057% increase in persistence
...
gridlev1 <- plotlev %>%
  ...
  # dplyr::mutate(grid_seed = sum(predicted_fecundity)) %>%
...
load(here::here("Persistence thresholds/Supp Bayesian models/Stan rdata/grid_pred_95CI.rdata"))
...
# THRESHOLD AT IF CI IS OVERLAPPING THRESHOLD, meaning grid upr just needs to be above threshold
gridlev$persistence[gridlev$grid_seed_upr >= gridlev$grid_n] <- c("yes") # threshold at grid_n, or grid n is between grid_seed_lwr and grid_seed_uppr
gridlev$persistence[gridlev$grid_seed_upr < gridlev$grid_n] <- c("no") # no persistence only if grid_seed_upr is less than 1

# also identify how many instances CI's overlap 1
overlap_dat <- data.frame(overlap = NA)
for(i in 1:length(gridlev$grid)){
  overlap_dat[i, 1] <- gridlev$grid_seed_lwr[i] <= gridlev$grid_n[i] & 
    gridlev$grid_n[i] <= gridlev$grid_seed_upr[i]
}
changed <- filter(overlap_dat, overlap == TRUE)[1]
length(changed$overlap)/length(gridlev$grid)*100 # 13.57143 increase in persistence
...
# join with the grid_pred data that has persistence:
load(here::here("Persistence thresholds/Supp Bayesian models/Stan rdata/site_pred_95CI.rdata"))
...
# THRESHOLD AT UPPER CI:
sitelev$persistence[sitelev$site_seed_upr >= sitelev$site_n] <- c("yes")
sitelev$persistence[sitelev$site_seed_upr < sitelev$site_n] <- c("no")

# also identify how many instances CI's overlap 1
overlap_dat <- data.frame(overlap = NA)
for(i in 1:length(sitelev$site)){
  overlap_dat[i, 1] <- sitelev$site_seed_lwr[i] <= sitelev$site_n[i] & 
    sitelev$site_n[i] <= sitelev$site_seed_upr[i]
}
changed <- filter(overlap_dat, overlap == TRUE)[1]
length(changed$overlap)/length(sitelev$site)*100 # 4.166667 increase in persistence
