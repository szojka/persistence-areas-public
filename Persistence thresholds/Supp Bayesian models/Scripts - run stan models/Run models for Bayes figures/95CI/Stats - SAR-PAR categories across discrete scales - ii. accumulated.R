...
#-------------------------------------------------------------------------------
# DESCRIPTION: Run stan model for Bayes version of Figure 4E (potential biases corrected)
#-------------------------------------------------------------------------------
...
# must run to reset data I used in 'averaged' script.
...
dt_o <- dt1 %>%
  dplyr::select(-persistence, -predicted_fecundity, -persist_final) 
...
# For changing the following GLMM models to brms models, often the random effect SDs are small, truncated  at 0. I determine if this is a realisitic estimate or if there is a model issue by looking at the Rhat (should not be >1.01) and the Bulk_ESS (should be greater than 1000).
...
# save model
save(m.p_brm, file = here::here("Persistence thresholds/Supp Bayesian models/Stan rdata/brms 95CI/m.p_brm.rdata"))
...
#save model
save(m.g_brm, file = here::here("Persistence thresholds/Supp Bayesian models/Stan rdata/brms 95CI/m.g_brm.rdata"))
...
# save model
save(m.s_brm, file = here::here("Persistence thresholds/Supp Bayesian models/Stan rdata/brms 95CI/m.s_brm.rdata"))
